...
namespace VirtuosoSkosSilverlight
{
    ...
    /// <summary>
    /// this class will be discontinued for the time being, because Virtuoso-Sparql implementation of our Virtuoso version is lacking the BIND-Methode
    /// this class should generate SPARQL-query-strings based on the raw-querys from the config file 
    /// this should replace hardcoded query-stings in the VirtuosoSkosQuery-Class
    /// </summary>
    public static class SkosQueryBuilder
    {
        private const string prefixSTR = "prefix";
        private static string xsd = VirtuosoSkosSilverlight_dll.xmlSchema;
        private static string queryPrefix = "PREFIX skos: <" + VirtuosoSkosSilverlight_dll.skosCore + "> ";
        private static string queryLanguageFilter = "FILTER langMatches( lang(?name), \"" + VirtuosoSkosSilverlight_dll.skosLanguage + "\" )";
        ...
        public static string getQuery(Querys query, OrderDirection order, string graph, string[] paramList)
        {
            ...
            string rawQuery = VirtuosoSkosSilverlight_dll.ResourceManager.GetResourceSet(CultureInfo.CurrentCulture, true, false).GetString("Query" + query.ToString());
